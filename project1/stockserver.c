...
{
    int listenfd, connfd,maxfdnum;
    int connfdarr[MAXCONNECT]={0,};
    socklen_t clientlen;
    ...
    maxfdnum=listenfd;
    fd_set readfds,reserve_readfds;
    FD_ZERO(&readfds);
    FD_ZERO(&reserve_readfds);
    FD_SET(listenfd,&reserve_readfds);
    while (1) {
        readfds=reserve_readfds;
        //printf("before select\n");
        Select(maxfdnum+1,&readfds,NULL,NULL,NULL);
        //printf("after select!\n");
        for(int i_fd=0;i<MAXCONNECT;i++){
            if(FD_ISSET(i_fd,&readfds)){
                if(i_fd==listenfd){
                    ...
                    FD_SET(connfd,&reserve_readfds);
                    if(connfd>maxfdnum)
                        maxfdnum=connfd;
                }
                else{
                    do_update_stock_table(i_fd);
                }
            }
        }
    }
